document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderDeals() {
        ...
        if (!grid) return;
        const deals = Array.isArray(db.deals) ? db.deals.slice(0, 3) : [];
        const dealCards = deals.map(d => createProductCard({
            _id: d._id,
            name: d.name,
            price: d.price,
        ...
        grid.innerHTML = dealCards.join('') || '<p>Hiện chưa có chương trình khuyến mãi.</p>';
    }
