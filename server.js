...
app.get('/api/deals', async (req, res) => {
  try {
    if (mongoose.connection.readyState !== 1) {
      return res.status(503).json({ message: 'MongoDB chưa kết nối. Vui lòng bật MongoDB và chạy lại seed.js nếu cần.' });
    }
...
app.put('/api/admin/deals/:id', async (req, res) => {
  try {
    ...
    if (!deal) return res.status(404).json({ message: 'Không tìm thấy khuyến mãi' });
    res.json(deal);
  } catch (err) {
    res.status(400).json({ message: err.message });
  }
...
app.get('/api/admin/deals/ping', (req, res) => {
  res.json({ ok: true });
});
...
// Nếu route /api/* không match ở trên, trả về 404 JSON (không trả HTML index)
app.use('/api', (req, res) => {
